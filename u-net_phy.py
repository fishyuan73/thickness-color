...
# 物理光谱模拟常数 —— 基于 absorbed.py 的 Beer-Lambert 吸收与 sRGB 渲染
K_DYE = torch.from_numpy((absorbed.rhodamine_epsilon(absorbed.LAM) * absorbed.c_dye).astype(np.float32))
S_WHITE = torch.from_numpy(absorbed.S_white.astype(np.float32))
WHITE_SCALE = torch.from_numpy(absorbed.WHITE_SCALE.astype(np.float32))
CIE = torch.from_numpy(np.stack([absorbed.CIE_X, absorbed.CIE_Y, absorbed.CIE_Z], axis=0).astype(np.float32))
SRGB_M = torch.from_numpy(absorbed.SRGB_M.astype(np.float32))
LOG10 = np.log(10.0).astype(np.float32)
...
def thickness_to_srgb_torch(pred):
    """根据 absorbed.py 的物理模型，将预测厚度渲染为 sRGB。

    pred: Tensor[N,1,H,W] 归一化厚度，输出 Tensor[N,3,H,W]。
    """
    h = torch.clamp(pred, min=0.0) * H_MAX
    if h.ndim == 4 and h.shape[1] == 1:
        h = h[:, 0]
    device = h.device
    k = K_DYE.to(device)
    s_white = S_WHITE.to(device)
    cie = CIE.to(device)
    white_scale = WHITE_SCALE.to(device)
    srgb_m = SRGB_M.to(device)

    flat = h.reshape(-1)
    rgb_flat = torch.empty((flat.shape[0], 3), dtype=torch.float32, device=device)
    chunk = 50000
    for i in range(0, flat.shape[0], chunk):
        hc = flat[i:i + chunk]
        t = torch.pow(10.0, -(hc[:, None] * k[None, :]))
        xyz = torch.matmul(t * s_white[None, :], cie.T) * white_scale[None, :]
        rgb_lin = torch.matmul(xyz, srgb_m.T)
        rgb = torch.where(rgb_lin <= 0.0031308,
                          12.92 * rgb_lin,
                          1.055 * torch.clamp(rgb_lin, min=0.0) ** (1.0 / 2.4) - 0.055)
        rgb_flat[i:i + chunk] = torch.clamp(rgb, 0.0, 1.0)

    rgb = rgb_flat.reshape(h.shape + (3,)).permute(0, 3, 1, 2)
    return rgb


def masked_rgb_loss(pred_rgb, rgb, mask):
    """仅对有效像素计算预测厚度渲染的 sRGB 与输入 RGB 之间的 MSE。"""
    mask3 = mask.expand_as(rgb)
    d = (pred_rgb - rgb) ** 2
    return (d * mask3).sum() / mask3.sum().clamp(min=1.0)
...
def plot_comparison(inp, gt, pr, err, save_path):
    ...
    真实厚度与预测厚度共用同一颜色刻度（vmin/vmax），便于直接比较。"""
...
def train(args, train_idx, val_idx, test_idx, images, thickness, mask):
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        run, nb = 0.0, 0
        run_phy, run_th = 0.0, 0.0
        for x, y, m in train_loader:
            x, y, m = x.to(device), y.to(device), m.to(device)
            pred = model(x)
            loss_th = masked_mse(pred, y, m)
            pred_rgb = thickness_to_srgb_torch(pred)
            loss_phy = masked_rgb_loss(pred_rgb, x, m)
            loss = loss_th + args.phy_weight * loss_phy
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            run += loss.item()
            run_th += loss_th.item()
            run_phy += loss_phy.item()
            nb += 1
        train_loss = run / nb
        train_th_loss = run_th / nb
        train_phy_loss = run_phy / nb

        val_rmse, val_mae = evaluate(model, val_loader, device)
        val_loss = float((val_rmse / H_MAX) ** 2)
        scheduler.step(val_loss)
        history['train_loss'].append(train_loss)
        history['train_thickness_loss'].append(train_th_loss)
        history['train_physics_loss'].append(train_phy_loss)
        history['val_loss'].append(val_loss)
        history['val_rmse_cm'].append(val_rmse)
        history['val_mae_cm'].append(val_mae)
        print(f'  第{epoch:3d}/{args.epochs}轮  总损失={train_loss:.5f}  '
              f'厚度MSE={train_th_loss:.5f}  物理RGB={train_phy_loss:.5f}  '
...
def main():
    ...
    ap.add_argument('--split-ratio', type=float, default=0.8,
                    help='训练集角度占比（其余为测试）')
    ap.add_argument('--phy-weight', type=float, default=0.1,
                    help='物理约束损失权重（厚度MSE + phy_weight * RGB约束）')
